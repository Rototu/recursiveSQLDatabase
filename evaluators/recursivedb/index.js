    ...
    const execute = ({ simpleOps, compositeOps, noOps }) => {
      ...
      for (const tName of simpleTables) {
        ...
        for (const op of ops) {
          ...
          if (!DB.isTableHashed(lhsSource, cName)) {
            DB.hashTable(lhsSource, cName, true);
          }

          // Get records satisfying where condition and put them into a temporary table
          const iterator = DB.getRecsFromHash(lhsSource, cName, operator, rhs);
        ...
        for (const pairOp of pairOps) {
          ...
          const tProj = proj
            ...
            .map((p) => {
              ...
              const srcTable = pCopy.colSrc[0];
              if (srcTable === lhs.parentTable && srcTable !== tempTableName) {
                pCopy.colSrc[0] = pairOp.lhsSource;
              } else if (
                srcTable === rhs.parentTable &&
                srcTable !== tempTableName
              ) {
                pCopy.colSrc[0] = pairOp.rhsSource;
              }
              return pCopy;
            });

          // use filtered down table if exists, otherwise use sorted one
          const src1 =
            tableNameMap.get(lhs.parentTable) !== lhs.parentTable
              ? tableNameMap.get(lhs.parentTable)
              : pairOp.lhsSource;
          const src2 =
            tableNameMap.get(rhs.parentTable) !== rhs.parentTable
              ? tableNameMap.get(rhs.parentTable)
              : pairOp.rhsSource;
          ...
          for (const joinedRec of joinedIterator) {
            if (
              lhs.parentTable === rhs.parentTable &&
      ...
      // check if any self join exists for the temporary table
      const keepAllFinalRecords =
        [...compositeOps.keys()].findIndex((key) => {
          const [t1, t2] = JSON.parse(key);
          return t1 === t2 && t2 === tempTableName;
        }) !== -1;
      ...
      const blockJoinIndependentTables = () => {
        if (independentTables.length === 1) {
          ...
          for (const finalRec of DB.getAllRecords(table)) {
            ...
            if (keepAllFinalRecords) {
              DB.insertUniqueRecordsById(finalTableName, [res], true);
            } else if (!DB.hasValue(resultTableName, '_id', res._id)) {
              DB.insertUniqueRecordsById(finalTableName, [res], true);
            }
          }
        ...
        do {
          ...
          for (const joinedRec of joinedIterator) {
            ...
            if (independentTables.length === 0) {
              // if this will be the final table only keep new records
              if (keepAllFinalRecords) {
                DB.insertUniqueRecordsById(tTableName, [recToInsert]);
              } else if (
                !DB.hasValue(resultTableName, '_id', recToInsert._id)
              ) {
                DB.insertUniqueRecordsById(tTableName, [recToInsert], true);
              }
            } else {
              DB.insertUniqueRecordsById(tTableName, [recToInsert]);
            }
...
function executeSingleQuery(query) {
  ...
  const tables = new Set(recTerm.tables);
  if (tables.has(recTableName)) tables.delete(recTableName);

  const copyTables = [];

  for (const op of recTerm.ops) {
    const lhs = op.lhs;
    const rhs = op.rhs;
    if (tables.has(lhs.parentTable)) {
      const sortedTableName = DB.copyIntoSortedTable(lhs.parentTable, lhs.name);
      op.lhsSource = sortedTableName;
      copyTables.push(sortedTableName);
    } else {
      op.lhsSource = lhs.parentTable;
    }
    if (rhs instanceof Column) {
      if (tables.has(rhs)) {
        const sortedTableName = DB.copyIntoSortedTable(
          rhs.parentTable,
          rhs.name
        );
        op.rhsSource = sortedTableName;
        copyTables.push(sortedTableName);
      } else {
        op.rhsSource = rhs.parentTable;
      }
    }
  }
  ...
  for (const copyTable of copyTables) {
    DB.drop(copyTable);
  }
}
